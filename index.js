...
signin.querySelector(".btn_login_primary").addEventListener("click",(event)=>{
...
async function fillModal(movie){
    ...
    modalNode.querySelector(".cast_block").querySelector("p").innerText=   currentCast; 
    modalNode.querySelector(".film_img").src="https://images.tmdb.org/t/p/original/"+movie.src;
...
async function getCast(movieId){
    let cast;
    try{
        let res=await fetch(`https://api.themoviedb.org/3/movie/${movieId}/credits?language=en-US`, options)
...
async function displayImages(images,container,data){
    ...
    data.results.forEach((movie)=>{
        ...
        if(movie.poster_path){
            addImage(`https://images.tmdb.org/t/p/original${movie.poster_path}`,container,infos);
        }
    });
}
...
async function addImage(imageURL,container,movie){
    ...
    divInfo.innerHTML=
    `   <h1 class="film_title">${movie.title}</h1>
